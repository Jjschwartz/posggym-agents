...
_BASE_AGENT_DIR = osp.join(osp.dirname(osp.abspath(__file__)), "agents")
...
for seed in range(5):
    ...
    for k in range(5):
        ...
        policy_dir = osp.join(
            _BASE_AGENT_DIR, parent_dir_name, "None", f"pi_{k}"
        )
        POLICY_SPECS[id] = load_rllib_policy_spec(id, policy_dir)

    # Add BR policy
    id = f"{ENV_NAME}/klrbr_k4_seed{seed}-v0"
    policy_dir = osp.join(_BASE_AGENT_DIR, parent_dir_name, "None", "pi_BR")
    POLICY_SPECS[id] = load_rllib_policy_spec(id, policy_dir)
